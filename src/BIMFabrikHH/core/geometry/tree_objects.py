...
from .primitive_objects import Cylinder, MeshRepresentation, Sphere
...
class Trunk:
    ...
    def as_geom(self) -> MeshRepresentation:
        ...
        cylinder = Cylinder(radius=self.radius, height=self.height, segments=self.segments)
    ...
    def as_product(self, model, builder) -> ifcopenshell.entity_instance:
        ...
        cylinder = Cylinder(radius=self.radius, height=self.height, segments=self.segments)
        mesh = cylinder.create_ifc_mesh(model, body)
...
class Crown:
    ...
    def as_product(self, model, builder) -> ifcopenshell.entity_instance:
        ...
        body = ifcopenshell.util.representation.get_context(model, "Model", "Body", "MODEL_VIEW")
        sphere = Sphere(radius=self.radius, detail=self.detail)
        mesh = sphere.create_ifc_mesh(model, body)
...
class TreeCluster:
    ...
    def build(self, model, body, storey, ifc_snippets):
        ...
        for idx, tree in enumerate(self.trees, 1):
            # Create a simple builder object for compatibility
            class SimpleBuilder:
                def __init__(self, model, body):
                    self.model = model
                    self.body = body

            builder = SimpleBuilder(model, body)
