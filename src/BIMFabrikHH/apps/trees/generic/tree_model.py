...
import BIMFabrikHH.data_models.pydantic_psets_tree as psets_module
...
from ....core.pset_utils import assign_psets_to_element, extract_psets_from_row
...
class Trunk:
    ...
    def build(self, builder, model, body, position, idx, ifc_snippets):
        # Create trunk entity
...
class Crown:
    ...
    def build(self, builder, model, body, position, idx, ifc_snippets):
        ...
        crown_mesh = geometry.add_mesh_representation(
            model, context=body, vertices=[vertices_crown], faces=[faces_crown], edges=None
        )
...
class Tree:
    ...
    def build(self, builder, model, body, storey, idx, ifc_snippets):

        # Create main tree entity
        tree_entity = root.create_entity(model, ifc_class="IfcBuildingElement", name=f"Baum_{idx:04d}")
