    ...
    try:
        ...
        with rasterio.open(str(file_path)) as src:
            ...
            if input_data is not None and hasattr(input_data, "bbox"):
                ...
                inside = (x_coords >= bbox[0]) & (x_coords <= bbox[2]) & (y_coords >= bbox[1]) & (y_coords <= bbox[3])
...
def process_terrain_folder_to_ifc(
    ...
    for file in tif_files:
        ...
        vertices, faces = extract_optimized_mesh_data(
            file_path, min_points=min_points, importance_threshold=importance_threshold, input_data=input_data
        )
    ...
    if move_to_origin and bbox is not None and combined_vertices:
        arr = np.array(combined_vertices)
        arr[:, 0] -= bbox[0]
        arr[:, 1] -= bbox[1]
        combined_vertices = arr.tolist()
        nullpunkt_x, nullpunkt_y = 0, 0
    elif bbox is not None:
        nullpunkt_x, nullpunkt_y = bbox[0], bbox[1]
    else:
        nullpunkt_x, nullpunkt_y = combined_vertices[0][0], combined_vertices[0][1] if combined_vertices else (0, 0)
