...
import pandas as pd
from BIMFabrikHH import BaumModeller, BoundingBoxParams, Component, Container, RequestParams
from BIMFabrikHH.apps.trees import DfColTree
...
def main():
    ...
    tif_path = str(Path(__file__).parent.parent / "assets" / "dgm1_32_558_9270_1_hh_2022.tif")
    ifc_path = modeller.create_tree_model_from_df(df, request_body, tif_path=tif_path)

    if ifc_path:
        logger.info(f"✓ Successfully created {ifc_path.name}\n{ifc_path.parent}", extra={"debug_category": "success"})
    else:
